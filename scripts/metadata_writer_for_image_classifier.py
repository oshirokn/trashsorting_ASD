...
_MODEL_INFO = {
    "FlowerModel.tflite":
        ModelSpecificInfo(
            name="Xception trash classifier",
            version="v1",
            image_width=320,
            image_height=320,
            image_min=0,
...
def main(_):
  model_file = "./model_wo_metadata/FlowerModel.tflite"
  model_basename = os.path.basename(model_file)
  if model_basename not in _MODEL_INFO:
    raise ValueError(
        "The model info for, {0}, is not defined yet.".format(model_basename))

  export_model_path = os.path.join("./model_w_metadata/", model_basename)
  ...
  populator = MetadataPopulatorForImageClassifier(
      export_model_path, _MODEL_INFO.get(model_basename), "./model_wo_metadata/labels.txt")#FLAGS.label_file)
  populator.populate()

  # Validate the output model file by reading the metadata and produce
  # a json file with the metadata under the export path
  displayer = _metadata.MetadataDisplayer.with_model_file(export_model_path)
  export_json_file = os.path.join("./model_w_metadata/",
